    ...
    public function getSystemErrors()
    {
        ...
        if (false === $res) {
            $errors[] = [
                'type' => 'error',
                'message' =>
                    'Data directory is not writable. Please make sure php can has write access to <doomeyehrm>/app/data',
            ];
        } else {
            ...
            if ($file_headers && count($status) > 1 && $status[1] != '404' && $status[1] != '403') {
                $errors[] = [
                    'type' => 'error',
                    'link' => 'https://doomeyehrm.gitbook.io/doomeyehrm/getting-started/securing-doomeyehrm-installation',
